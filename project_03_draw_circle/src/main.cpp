...
#pragma endregion
	...
	vertices[0] = centerPoint[0];
	vertices[1] = centerPoint[1];
	vertices[2] = centerPoint[2];

	for (int i = 0; i <= numSegments; i++) {
		float theta = 2.0f * 3.14159 * static_cast<float>(i) / static_cast<float>(numSegments);
		vertices[(i + 1) * 3] = centerPoint[0] + radius * cos(theta);
		vertices[(i + 1) * 3 + 1] = centerPoint[1] + radius * sin(theta);
		vertices[(i + 1) * 3 + 2] = centerPoint[2];
	}
